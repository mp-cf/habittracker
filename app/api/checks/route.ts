...
export async function POST(request: NextRequest) {
  ...
  try {
    const { habit_id, day, completed } = await request.json();
    if (!habit_id || day === undefined || completed === undefined) {
      return NextResponse.json({ error: 'habit_id, day, and completed are required' }, { status: 400 });
    }
    const check = await prisma.check.create({
      data: { habit_id, day: parseInt(day), completed: Boolean(completed) },
    });
